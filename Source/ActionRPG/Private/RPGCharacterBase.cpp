...
void ARPGCharacterBase::StartDecreaseStamina(float StaminaCost, float decreaseTime)
{
	if (AbilitySystemComponent && AttributeSet)
	{
		GetWorld()->GetTimerManager().ClearTimer(StaminaRegenTimerHandle);

		GetWorld()->GetTimerManager().SetTimer(StaminaTimerHandle, [this, StaminaCost, decreaseTime]() {
			const float CurrentStamina = AttributeSet->GetStamina();

			const float NewStamina = FMath::Clamp(CurrentStamina - StaminaCost, 0.0f, AttributeSet->GetMaxStamina());

			AbilitySystemComponent->SetNumericAttributeBase(AttributeSet->GetStaminaAttribute(), NewStamina);

			HandleStaminaChanged(StaminaCost);
			},1.0f, true, decreaseTime);
	}
}

void ARPGCharacterBase::StopDecreaseStamina(float RegenCost, float regenTime)
{
	if (AbilitySystemComponent && AttributeSet)
	{
		GetWorld()->GetTimerManager().ClearTimer(StaminaTimerHandle);

		GetWorld()->GetTimerManager().SetTimer(StaminaRegenTimerHandle, [this, RegenCost, regenTime]() {
				const float CurrentStamina = AttributeSet->GetStamina();

				const float NewStamina = FMath::Clamp(CurrentStamina + RegenCost, 0.0f, AttributeSet->GetMaxStamina());

				AbilitySystemComponent->SetNumericAttributeBase(AttributeSet->GetStaminaAttribute(), NewStamina);

				HandleStaminaChanged(RegenCost);
			}, 1.0f, true, regenTime);
	}
}
...
void ARPGCharacterBase::HandleStaminaChanged(float DeltaValue)
{
	if (bAbilitiesInitialized)
	{
		OnStaminaChanged(DeltaValue);
	}
}
